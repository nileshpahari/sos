...
impl Calculator {
    ...
    pub fn addition(&mut self, x: i64, y: i64) -> Option<i64> {
        ...
        if let Some(result) =
            OperationType::Addition.perform(operation.first_num, operation.second_num)
        {
    ...
    pub fn subtraction(&mut self, x: i64, y: i64) -> Option<i64> {
        ...
        if let Some(result) =
            OperationType::Subtraction.perform(operation.first_num, operation.second_num)
        {
            self.history.push(operation);
            Some(result)
        } else {
    ...
    pub fn multiplication(&mut self, x: i64, y: i64) -> Option<i64> {
        ...
        if let Some(result) =
            OperationType::Multiplication.perform(operation.first_num, operation.second_num)
        {
            self.history.push(operation);
            Some(result)
        } else {
