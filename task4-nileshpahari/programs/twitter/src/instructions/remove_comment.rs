...
pub struct RemoveCommentContext<'info> {
    ...
    #[account(mut, close=comment_author, seeds=[COMMENT_SEED.as_bytes(), comment_author.key().as_ref(), {hash(comment.content.as_bytes()).to_bytes().as_ref()}, comment.parent_tweet.key().as_ref()],bump=comment.bump)]
